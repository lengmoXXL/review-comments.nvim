...
describe("review-comments.keymaps buffer setup", function()
  ...
  it("sets leader-style comment keymaps", function()
    ...
    assert.is_true(has_key("<localleader>ct"))
  end)
  ...
  it("does not set disabled tmux keymap", function()
    config.setup({ keymaps = { send_tmux = false } })
    keymaps.setup_buffer(bufnr)

    assert.is_false(has_key("<localleader>ct"))
  end)
end)
